# CDPN v6: Consistency + Energy gradient (direct gradient through WM)
...
def energy(s):
    """Total mechanical energy at state s (uses G0 for consistency with WM1)."""
...
print("="*70)
print("Consistency + Energy Gradient on Acrobot")
...
# Combined training: consistency + energy gradient
print("\n[2] Combined: consistency + lam * energy_gradient")
for lam in [0.0, 0.0001, 0.001, 0.01]:
    ...
    for ep in range(1,31):
        ...
        for _ in range(100):
            ...
            if lam>0:
                # Energy gradient (negative because we want to maximize energy)
                e=energy(s_pred)
                loss=loss-lam*e.mean()
        ...
        if ep%10==0:
            ...
            print(f"    lam={lam:.0e} ep{ep:2d} g={G0}={g15}/20  g=9.8={g9}/20  loss={tl/100:.6f}")
    g15=evaluate(pi,g=G0);g9=evaluate(pi,g=9.8)
    print(f"  lam={lam:.0e}: FINAL g={G0}={g15}/20  g=9.8={g9}/20")
