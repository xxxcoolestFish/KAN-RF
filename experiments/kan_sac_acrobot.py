"""KAN-SAC vs MLP-SAC on Acrobot (harder task)."""
...
G=9.8;L1=L2=1.;LC1=LC2=.5;I1=I2=1.;DT=.05;MAX_V1=6.;MAX_V2=8.;TARGET_H=1.0

def acrobot_step(sr,a,m1=1.,m2=1.):
...
def eval_ac(ag,n=10,m1=1.,m2=1.):
    ...
    for t in range(n):
        ...
        for _ in range(500):
            a=ag.act(s.squeeze(0),dt=True);s=acrobot_step(s,torch.tensor([a]),m1,m2)
            if tip_h(s).item()>TARGET_H:sc+=1;break
    return sc

def train_ph(ag,env_fn,bu,steps,m1,m2,label):
    o=torch.tensor([[np.cos(th1:=-.05+.1*np.random.rand()),np.sin(th1),np.cos(th2:=-.05+.1*np.random.rand()),np.sin(th2),0.,0.]],dtype=torch.float32)
    t0=time.time()
    for s in range(1,steps+1):
        a=ag.act(o.squeeze(0));o2=acrobot_step(o,torch.tensor([a]),m1,m2)
    ...
    sc=eval_ac(ag,10,m1,m2);print(f"  {label}: {sc}/10 time={time.time()-t0:.0f}s");return sc

print("KAN-SAC vs MLP-SAC on Acrobot");t0=time.time()
print("\n[MLP] m1=m2=1 (10000 steps)")
ma=SAC(lambda:MLPA(),lambda:MLPQ());mb=Buf()
mg1=train_ph(ma,None,mb,10000,1.,1.,"MLP m=1")
print("\n[KAN] m1=m2=1 (10000 steps)")
ka=SAC(lambda:KANA(),lambda:KANQ());kb=Buf()
kg1=train_ph(ka,None,kb,10000,1.,1.,"KAN m=1")
print("\n[ADAPT] Both continue on m=2 (20000 more steps)")
mg2=train_ph(ma,None,mb,20000,2.,2.,"MLP m=2")
kg2=train_ph(ka,None,kb,20000,2.,2.,"KAN m=2")
