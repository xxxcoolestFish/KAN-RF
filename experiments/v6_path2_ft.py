...
PROBES=build_probes()
class SepCondPolicy(nn.Module):
    def __init__(self,h_dim=16):
        super().__init__();self.s_encoder=ProtoKAN([6,h_dim],n_prototypes=16)
        self.z_project=nn.Linear(2,h_dim);self.decoder=ProtoKAN([h_dim,1],n_prototypes=16)
    def forward(self,s,z):
        h_s=self.s_encoder(s);h_z=self.z_project(z)
        return torch.tanh(self.decoder(h_s+h_z))
...
def fine_tune_from(wm_src,x,y,steps=500,lr=1e-4):
    wm=ProtoKAN([7,16,6],n_prototypes=16).to(DEVICE)
    for l in wm.layers:l.log_sigma.data.fill_(-1.5)
    wm.load_state_dict(wm_src.state_dict());wm.train()
...
def eval_sep(policy,wm0,wm1,pca,z_info,g=14.7,n=20):
    z=get_z(wm1,wm0,pca);zn=(z-z_info[0])/(z_info[1]+1e-8)
    zt=torch.tensor(zn,dtype=torch.float32).unsqueeze(0);sc=0
    for t in range(n):
    ...
    return sc
print("="*70);print("Path 2: SepCondPolicy + FT (continual learning)");print("="*70);t0=time.time()
...
for g in TRAIN_G+[TEST_G]:
    if g==9.8:continue
    wms[g]=fine_tune_from(wm0,data[g][0],data[g][1])
    e=mse(wms[g](data[g][0][:1000]),data[g][1][:1000]).item()
    print(f"  g={g:4.1f}: FT MSE={e:.6f}")
print("\n[3] PCA + z norm")
d_all=np.array([d_vec(wms[g],wm0) for g in TRAIN_G])
pca=PCA(n_components=2).fit(d_all)
z_all=np.array([get_z(wms[g],wm0,pca) for g in TRAIN_G])
zm,zs=z_all.mean(axis=0),z_all.std(axis=0);z_info=(zm,zs)
for g in TRAIN_G+[TEST_G]:
    z=get_z(wms[g],wm0,pca);zn=(z-zm)/(zs+1e-8)
    print(f"  g={g:4.1f}: zn=[{zn[0]:+.3f},{zn[1]:+.3f}]")
print(f"  EVR: {pca.explained_variance_ratio_}")
print("\n[4] Pre-train SepCondPolicy (50 ep)")
policy=SepCondPolicy();opt=torch.optim.Adam(policy.parameters(),lr=3e-4)
S_G=torch.tensor([[-1.,0.,1.,0.,0.,0.]]).to(DEVICE)
for ep in range(1,51):
    tl=0.0
    for _ in range(60):
        g=np.random.choice(TRAIN_G);wm=wms[g]
        z=get_z(wms[g],wm0,pca);zn=(z-zm)/(zs+1e-8);zt=torch.tensor(zn,dtype=torch.float32).unsqueeze(0)
    ...
    if ep%25==0:
        g9=eval_sep(policy,wm0,wms[9.8],pca,z_info,g=9.8,n=5)
        gt=eval_sep(policy,wm0,wms[12.0],pca,z_info,g=12.0,n=5)
        print(f"    ep{ep:2d} loss={tl/60:.4f} g=9.8={g9}/5 g=12={gt}/5")
# Save pre-trained policy
policy_pt=SepCondPolicy();policy_pt.load_state_dict(policy.state_dict())
g15_zs=eval_sep(policy,wm0,wms[TEST_G],pca,z_info,g=TEST_G,n=20)
print(f"  ZERO-SHOT (pre-trained): g=14.7={g15_zs}/20")
print("\n[5] Continual learning: fine-tune pi on g=14.7 (10 ep)")
wm1=wms[TEST_G];z14=torch.tensor((get_z(wm1,wm0,pca)-zm)/(zs+1e-8),dtype=torch.float32).unsqueeze(0)
...
for ep in range(1,11):
    ...
    for _ in range(60):
        s=random_states(96)
        # Loss 1: BPTT
        opt2.zero_grad();loss=torch.tensor(0.,device=DEVICE);sc=s.clone()
        for t in range(3):
            a=policy(sc,z14.expand(sc.shape[0],-1));sc=wm1(torch.cat([sc,a],-1))
            loss=loss+(0.9**t)*(sc-S_G.expand(96,-1)).pow(2).sum(dim=-1).mean()
        loss.backward();torch.nn.utils.clip_grad_norm_(policy.parameters(),10.);opt2.step();tl+=loss.item()
    if ep%5==0:
        g15=eval_sep(policy,wm0,wms[TEST_G],pca,z_info,g=TEST_G,n=20)
        g9=eval_sep(policy,wm0,wms[TEST_G],pca,z_info,g=9.8,n=20)
        print(f"    ep{ep:2d} BPTT_loss={tl/60:.4f} g=14.7={g15}/20 g=9.8={g9}/20")
g15_ft=eval_sep(policy,wm0,wms[TEST_G],pca,z_info,g=TEST_G,n=20)
g9_ft=eval_sep(policy,wm0,wms[TEST_G],pca,z_info,g=9.8,n=20)
print(f"  FT: g=14.7={g15_ft}/20 g=9.8={g9_ft}/20")
print("\n[6] Compare: consistency fine-tuning (10 ep)")
policy.load_state_dict(policy_pt.state_dict())
opt3=torch.optim.Adam(policy.parameters(),lr=1e-4)
for p in wm1.parameters():p.requires_grad=False;wm1.eval()
for ep in range(1,11):
    tl=0.0
    for _ in range(60):
        s=random_states(96);opt3.zero_grad()
        with torch.no_grad():a0=policy_pt(s,z14.expand(s.shape[0],-1));tgt=wm1(torch.cat([s,a0],-1))
        a=policy(s,z14.expand(s.shape[0],-1));sp=wm1(torch.cat([s,a],-1))
        loss=mse(sp,tgt.detach());loss.backward();opt3.step();tl+=loss.item()
    if ep%5==0:
        g15=eval_sep(policy,wm0,wms[TEST_G],pca,z_info,g=TEST_G,n=20)
        print(f"    ep{ep:2d} cons_loss={tl/60:.6f} g=14.7={g15}/20")
g15_cf=eval_sep(policy,wm0,wms[TEST_G],pca,z_info,g=TEST_G,n=20)
print(f"  CONS FT: g=14.7={g15_cf}/20")
print(f"\n{'='*70}\nSUMMARY\n{'='*70}")
print(f"  Zero-shot (no FT):     {g15_zs}/20")
print(f"  BPTT FT (10 ep):       {g15_ft}/20")
print(f"  Consistency FT (10 ep): {g15_cf}/20")
