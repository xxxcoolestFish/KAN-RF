# CDPN v6: On-policy grid search distillation (DAgger style)
...
def collect_onpolicy(policy,g=14.7,n_episodes=10,max_steps=200):
    """Run policy and collect visited states."""
    states=[]
    for _ in range(n_episodes):
        ...
        for st in range(max_steps):
            with torch.no_grad():a=policy(s).item()
            states.append(s[0].clone())
            s=acrobot_step(s,torch.tensor([[a]]),g)
            if tip_h(s).item()>TARGET_H:break
    return torch.stack(states)

def grid_labels(wm,states):
    """Grid search: best action via combined score."""
    batch=states.shape[0];n_grid=41
    a_grid=torch.linspace(-1.,1.,n_grid).unsqueeze(-1)
    s_exp=states.unsqueeze(1).expand(-1,n_grid,-1)
    a_exp=a_grid.unsqueeze(0).expand(batch,-1,-1)
    sa=torch.cat([s_exp,a_exp],dim=-1)
    S_G=torch.tensor([[-1.,0.,1.,0.,0.,0.]]).to(DEVICE)
    with torch.no_grad():
        s_next=wm(sa.view(-1,7)).view(batch,n_grid,6)
        sd=(s_next-S_G.view(1,1,6)).pow(2).sum(dim=-1)
        h_next=tip_h(s_next.view(-1,6)).view(batch,n_grid)
        score=-sd+0.5*h_next
        _,idx=score.max(dim=-1)
    return a_grid[idx].squeeze(-1)

print("="*70)
print("On-policy Grid Search Distillation (DAgger)")
...
succ0_9=evaluate(policy0,g=9.8);succ0_15=evaluate(policy0,g=14.7)
print(f"  pi0: g=9.8={succ0_9}/20  g=14.7={succ0_15}/20")
...
# Phase 2a: Consistency warmup
print("\n[2a] Consistency warmup (10 epochs)")
...
for ep in range(1,11):
    ...
    for _ in range(100):
        ...
        with torch.no_grad():a_old=policy0(s);s_target=wm0(torch.cat([s,a_old],-1))
        a_new=policy(s);s_pred=wm1(torch.cat([s,a_new],-1))
        loss=mse(s_pred,s_target)
        loss.backward();torch.nn.utils.clip_grad_norm_(policy.parameters(),10.)
        opt.step();tl+=loss.item()
g15_c=evaluate(policy,g=14.7);g9_c=evaluate(policy,g=9.8)
print(f"  Cons: g=14.7={g15_c}/20  g=9.8={g9_c}/20")

# Phase 2b: On-policy DAgger distillation
print("\n[2b] DAgger-style on-policy distillation (5 rounds)")
op=torch.optim.Adam(policy.parameters(),lr=1e-4)
for rnd in range(1,6):
    # Collect on-policy states
    onp_states=collect_onpolicy(policy,g=14.7,n_episodes=20,max_steps=300)
    print(f"  Round {rnd}: {len(onp_states)} states collected")
    
    # Generate grid labels
    a_star=grid_labels(wm1,onp_states)
    
    # Train on these (state, label) pairs
    for sub_ep in range(5):
        idx=torch.randint(0,len(onp_states),(128,))
        s=onp_states[idx]
        op.zero_grad()
        loss=mse(policy(s),a_star[idx].unsqueeze(-1))
        loss.backward();torch.nn.utils.clip_grad_norm_(policy.parameters(),10.)
        op.step()
    
    g15=evaluate(policy,g=14.7);g9=evaluate(policy,g=9.8)
    print(f"    g=14.7={g15}/20  g=9.8={g9}/20  loss={loss.item():.6f}")
    if g15>=18:print(f"    -> Early success!");break

g15_d=evaluate(policy,g=14.7);g9_d=evaluate(policy,g=9.8)

# Full BPTT baseline
print("\n[3] FULL BPTT baseline (20 ep, H=3)")
pi_f=ProtoPolicy();pi_f.load_state_dict(policy0.state_dict())
opt_f=torch.optim.Adam(pi_f.parameters(),lr=1e-4)
S_G=torch.tensor([[-1.,0.,1.,0.,0.,0.]]).to(DEVICE)
for ep in range(1,21):
    ...
    for _ in range(100):
        ...
        for t in range(3):
            a=pi_f(sc);sc=wm1(torch.cat([sc,a],-1))
            loss=loss+(0.9**t)*(sc-S_G.expand(128,-1)).pow(2).sum(dim=-1).mean()
        loss.backward();torch.nn.utils.clip_grad_norm_(pi_f.parameters(),10.)
        opt_f.step();tl+=loss.item()
    if ep%10==0:g15=evaluate(pi_f,g=14.7);g9=evaluate(pi_f,g=9.8);print(f"    ep{ep:2d} g=14.7={g15}/20  g=9.8={g9}/20")
g15_f=evaluate(pi_f,g=14.7);g9_f=evaluate(pi_f,g=9.8)

print(f"\n{'='*70}\nFINAL RESULTS\n{'='*70}")
print(f"  {'Method':30s}  {'g=14.7':>8s}  {'g=9.8':>8s}")
print(f"  {'pi0 zero-shot':30s}  {succ0_15:8d}  {succ0_9:8d}")
print(f"  {'Consistency':30s}  {g15_c:8d}  {g9_c:8d}")
print(f"  {'DAgger Distill':30s}  {g15_d:8d}  {g9_d:8d}")
print(f"  {'Full BPTT':30s}  {g15_f:8d}  {g9_f:8d}")
