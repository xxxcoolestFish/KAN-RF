"""CartPole SAC with WM monitoring."""
...
class SAC:
    ...
    def upd(self,buf,b=128):
        ...
        with torch.no_grad():
            a_,lp_=self.a.sample(s_);q=torch.min(self.t1(s_,a_),self.t2(s_,a_))-self.al*lp_;y=r+self.g*(1-d)*q
...
cpx=torch.cat(cpx);cpy=torch.cat(cpy);nt_=int(len(cpx)*.85)
mse=nn.MSELoss();wm=ProtoKAN([5,16,4],n_prototypes=16).to(DEVICE)
...
for st in range(1,50001):
    ...
    r2=-(sn[0,2].item()**2+0.1*sn[0,0].item()**2+0.001*float(a)**2);bu.push(o.squeeze(0).numpy(),float(a),r2,sn.squeeze(0).numpy(),d)
    if st%2==0:ag.upd(bu)
    o=sn
    if d:o=torch.tensor([[0.,0.,rng.uniform(-.05,.05)/THS,0.]],dtype=torch.float32)
    if st%5000==0:
        with torch.no_grad():s,a,_,s_,_=bu.sample(128);wp=wm(torch.cat([s,a],-1));we=mse(wp,s_).item()
        sc=0
        for _ in range(10):
            th_=rng.uniform(-.05,.05);s=torch.tensor([[0.,0.,th_/THS,0.]],dtype=torch.float32)
            for _s in range(500):
                a2=ag.act(s.squeeze(0),dt=True)
