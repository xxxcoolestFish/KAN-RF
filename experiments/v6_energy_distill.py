# CDPN v6: Energy-based grid search distillation
...
def energy(s):
    """Total mechanical energy of Acrobot system."""
    ...
    # Potential (relative to bottom, theta=0)
...
def grid_labels_energy(wm,states,n_grid=41,beta=0.1):
    """a* = argmax [E(WM(s,a)) - beta*a^2]. Energy breaks bottom symmetry!"""
    ...
    with torch.no_grad():
        ...
        E_next=energy(s_next.view(-1,6)).view(batch,n_grid)
        score=E_next-beta*a_grid.squeeze(-1).pow(2)  # energy - regularization
        _,idx=score.max(dim=-1)
    return a_grid[idx].squeeze(-1)

def grid_labels_tip(wm,states,n_grid=41):
    """a* = argmax [tip_height change] - baseline comparison."""
    batch=states.shape[0]
    a_grid=torch.linspace(-1.,1.,n_grid).unsqueeze(-1)
    s_exp=states.unsqueeze(1).expand(-1,n_grid,-1)
    a_exp=a_grid.unsqueeze(0).expand(batch,-1,-1)
...
print("="*70)
print("Energy-based Grid Search Distillation")
...
print("\n[2a] Consistency (10 ep)")
for p in [wm1,wm0,policy0]:[setattr(pp,'requires_grad',False) for pp in p.parameters()]
...
for ep in range(1,11):
    ...
    for _ in range(100):
        ...
        loss.backward();torch.nn.utils.clip_grad_norm_(policy.parameters(),10.)
        opt.step();tl+=loss.item()
g15_c=evaluate(policy,g=G0);g9_c=evaluate(policy,g=9.8)
print(f"  Cons: g={G0}={g15_c}/20  g=9.8={g9_c}/20  loss={tl/100:.6f}")

# Test: energy-based vs tip-based grid search
for name,label_fn in [("EnergyGrid",grid_labels_energy),("TipGrid",grid_labels_tip)]:
    pi=ProtoPolicy();pi.load_state_dict(policy.state_dict())
    ...
    for ep in range(1,21):
        ...
        if ep%10==0:
            ...
            print(f"    ep{ep:2d} g={G0}={g15}/20  g=9.8={g9}/20  loss={tl/100:.6f}")
    g15_f=evaluate(pi,g=G0);g9_f=evaluate(pi,g=9.8)
    print(f"  FINAL: g={G0}={g15_f}/20  g=9.8={g9_f}/20  loss={tl/100:.6f}")
