...
print("="*70);print("Self-consistent loss: L = ||WM1 - WM0||^2");print("="*70);t0=time.time()
print("\n[1] Data + WMs")
X9,Y9=gen_data(5000,g=9.8);X15,Y15=gen_data(5000,g=14.7)
wm0,_=train_wm_lbfgs(X9,Y9,80)
wm1=fine_tune_from(wm0,X15,Y15)
...
# Test 1: Pure self-consistent
print("\n[2] Pure L_self (30 ep)")
pi=ProtoPolicy();op=torch.optim.Adam(pi.parameters(),lr=1e-3)
for ep in range(1,31):
    tl=0.0;ma=0.0
    for _ in range(100):
        s=random_states(128);op.zero_grad()
        a=pi(s);s1=wm1(torch.cat([s,a],-1));s0=wm0(torch.cat([s,a],-1))
        loss=mse(s1,s0);loss.backward();torch.nn.utils.clip_grad_norm_(pi.parameters(),10.)
        op.step();tl+=loss.item();ma+=a.abs().mean().item()
    if ep%10==0:
        g15=evaluate(pi,g=14.7);g9=evaluate(pi,g=9.8)
        print(f"    ep{ep:2d} loss={tl/100:.6f} mean|a|={ma/100:.4f} g=14.7={g15}/20 g=9.8={g9}/20")
g15_s=evaluate(pi,g=14.7);g9_s=evaluate(pi,g=9.8)
print(f"  L_self: g=14.7={g15_s}/20 g=9.8={g9_s}/20")

# Test 2: Self-consistent + goal
print("\n[3] L_self + 0.01*L_goal (30 ep)")
pi2=ProtoPolicy();op2=torch.optim.Adam(pi2.parameters(),lr=1e-3)
for ep in range(1,31):
    tl=0.0
    for _ in range(100):
        s=random_states(128);op2.zero_grad()
        a=pi2(s);s1=wm1(torch.cat([s,a],-1));s0=wm0(torch.cat([s,a],-1))
        l_self=mse(s1,s0);l_goal=(s1-S_G.expand(128,-1)).pow(2).sum(dim=-1).mean()
        loss=l_self+0.01*l_goal;loss.backward();torch.nn.utils.clip_grad_norm_(pi2.parameters(),10.)
        op2.step();tl+=loss.item()
    if ep%10==0:
        g15=evaluate(pi2,g=14.7);g9=evaluate(pi2,g=9.8)
        print(f"    ep{ep:2d} loss={tl/100:.6f} g=14.7={g15}/20 g=9.8={g9}/20")
g15_sg=evaluate(pi2,g=14.7);g9_sg=evaluate(pi2,g=9.8)
print(f"  L_self+goal: g=14.7={g15_sg}/20 g=9.8={g9_sg}/20")

# Test 3: Self-consistent + goal + energy
print("\n[4] L_self + 0.01*L_goal + 0.001*(-E) (30 ep)")
pi3=ProtoPolicy();op3=torch.optim.Adam(pi3.parameters(),lr=1e-3)
for ep in range(1,31):
    tl=0.0
    for _ in range(100):
        s=random_states(128);op3.zero_grad()
        a=pi3(s);s1=wm1(torch.cat([s,a],-1));s0=wm0(torch.cat([s,a],-1))
        l_self=mse(s1,s0);l_goal=(s1-S_G.expand(128,-1)).pow(2).sum(dim=-1).mean()
        loss=l_self+0.01*l_goal-0.001*energy(s1).mean()
        loss.backward();torch.nn.utils.clip_grad_norm_(pi3.parameters(),10.)
        op3.step();tl+=loss.item()
    if ep%10==0:
        g15=evaluate(pi3,g=14.7);g9=evaluate(pi3,g=9.8);e_m=energy(s1).mean().item()
        print(f"    ep{ep:2d} loss={tl/100:.6f} E={e_m:.2f} g=14.7={g15}/20 g=9.8={g9}/20")
g15_sge=evaluate(pi3,g=14.7);g9_sge=evaluate(pi3,g=9.8)
print(f"  L_self+goal+E: g=14.7={g15_sge}/20 g=9.8={g9_sge}/20")
