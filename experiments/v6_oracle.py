# CDPN v6: Oracle experiment - true dynamics as perfect WM
...
# True Acrobot dynamics (fully differentiable via torch)
def true_dynamics(sr,a,g):
...
def gen_data(n=5000,g=9.8):
    ...
    for _ in range(n):
        ...
        sn=true_dynamics(s,torch.tensor([[a]]),g)
...
def evaluate(policy,g=9.8,n=20):
    ...
    for t in range(n):
        ...
        for _ in range(500):
            ...
            s=true_dynamics(s,torch.tensor([[a]]),g)
...
print("="*70)
print("Oracle: True Dynamics as Perfect WM on Acrobot")
print("="*70);t0=time.time()

# Phase 1: Train WM0 on g=9.8 and pi0
print("\n[1] Train WM0 (g=9.8) and pi0")
X9,Y9=gen_data(5000,g=9.8)
wm0,bv=train_wm_lbfgs(X9,Y9,80)
...
# Phase 2: Consistency adaptation with TRUE dynamics as WM (g=14.7)
print("\n[2] Consistency with TRUE dynamics (g=14.7) - 10 epochs")
for p in wm0.parameters():p.requires_grad=False
for p in policy0.parameters():p.requires_grad=False
wm0.eval();policy0.eval()
...
for ep in range(1,11):
    ...
    for _ in range(100):
        s=random_states(128)
        opt.zero_grad()
        # Target from WM0(s, pi0(s))
        with torch.no_grad():
            a_old=policy0(s)
            s_target=wm0(torch.cat([s,a_old],-1))
        # Prediction from TRUE dynamics (perfect WM for g=14.7)
        a_new=policy(s)
        s_pred=true_dynamics(s,a_new,g=14.7)
...
g15_oracle=evaluate(policy,g=14.7);g9_oracle=evaluate(policy,g=9.8)
print(f"  ORACLE: g=14.7={g15_oracle}/20  g=9.8={g9_oracle}/20")

# Phase 3: Compare with standard WM1 consistency
print("\n[3] Standard WM1 consistency (for comparison)")
X15,Y15=gen_data(5000,g=14.7)
...
wm1.train();owm=torch.optim.Adam(wm1.parameters(),lr=1e-4)
for _ in range(500):
    idx=torch.randint(0,len(X15),(256,));l=mse(wm1(X15[idx].to(DEVICE)),Y15[idx].to(DEVICE))
    owm.zero_grad();l.backward();owm.step()
wm1.eval()
for p in wm1.parameters():p.requires_grad=False

policy2=ProtoPolicy()
policy2.load_state_dict(policy0.state_dict())
opt2=torch.optim.Adam(policy2.parameters(),lr=1e-3)
for ep in range(1,11):
    tl=0.0
    for _ in range(100):
        s=random_states(128);opt2.zero_grad()
        with torch.no_grad():
            a_old=policy0(s);s_target=wm0(torch.cat([s,a_old],-1))
        a_new=policy2(s);s_pred=wm1(torch.cat([s,a_new],-1))
        loss=mse(s_pred,s_target)
        loss.backward();torch.nn.utils.clip_grad_norm_(policy2.parameters(),10.)
        opt2.step();tl+=loss.item()
g15_std=evaluate(policy2,g=14.7);g9_std=evaluate(policy2,g=9.8)
print(f"  STANDARD: g=14.7={g15_std}/20  g=9.8={g9_std}/20")

print(f"\n{'='*70}")
print("ORACLE RESULT")
print(f"  pi0 zero-shot:     g=14.7={succ0_15}/20  g=9.8={succ0_9}/20")
print(f"  ORACLE (true WM):  g=14.7={g15_oracle}/20  g=9.8={g9_oracle}/20")
print(f"  STANDARD (WM1):    g=14.7={g15_std}/20  g=9.8={g9_std}/20")
print(f"  Gap: {g15_oracle - g15_std}/20")
