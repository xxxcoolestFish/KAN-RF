"""CDPNv5 adaptation: WM detects change -> clear buffer -> SAC recovers to 100%."""
import torch,torch.nn as nn,numpy as np,time,os,sys,gymnasium as gym;from collections import deque
...
LOG_MIN=-5;LOG_MAX=2;mse=nn.MSELoss()
...
print("="*60);print("CDPNv5: WM-guided adaptation");print("="*60);t0=time.time()

# [1] Train WM
print("\n[1] Train WM on g=10");X,Y=generate_pendulum_data(5000);wn,_=train_wm(X.to(DEVICE),Y.to(DEVICE))

# [2] SAC on g=10
print("\n[2] SAC on g=10 (10000 steps)");ag=SAC();bu=Buf();env=gym.make("Pendulum-v1");o,_=env.reset()

for st in range(1,10001):
...
g10_eval=eval_sac(ag,env);print(f"  Step 10000 eval_g10={g10_eval}/20")

# [3] Fine-tune WM on SAC data
print("\n[3] Fine-tune WM on g=10 SAC data")
optwm=torch.optim.Adam(wn.parameters(),lr=1e-4);wn.train()
for _ in range(200):
    s,a,_,s_,_=bu.sample(128);wp=wn(torch.cat([s,a],-1));optwm.zero_grad();mse(wp,s_).backward();optwm.step()
wn.eval();s,a,_,s_,_=bu.sample(128);wp=wn(torch.cat([s,a],-1));pre_change_err=mse(wp,s_).item()
print(f"  WM error before gravity change: {pre_change_err:.6f}")

# [4] Change gravity
print("\n[4] GRAVITY CHANGE: g=10 -> g=15");env15=gym.make("Pendulum-v1");o,_=env15.reset()
N=0;wm_err_roll=[]
for st in range(10001,20001):
    ...
    if st%50==0:
        with torch.no_grad():s,a,_,s_,_=bu.sample(64);we=mse(wn(torch.cat([s,a],-1)),s_).item()
        wm_err_roll.append(we)
        if len(wm_err_roll)==5:
            avg_we=sum(wm_err_roll)/5
            if avg_we>pre_change_err*5 and avg_we>.1:
                print(f"  Step{st}: WM detected change! error={avg_we:.6f} (>threshold). Clearing buffer...")
                bu.clear();wm_err_roll=[]
                # Reset exploration
                ag.la.data=torch.tensor(np.log(.5));print("  SAC exploration reset (alpha=0.5)")
g15_eval=eval_sac(ag,env15);print(f"  Final eval_g15={g15_eval}/20")
