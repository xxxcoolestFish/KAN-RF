...
def eval_sep(policy,wm0,wm1,pca,z_info,g=14.7,n=20):
    ...
    return sc
print("="*70);print("Path 2: SepCondPolicy (50 ep)");print("="*70);t0=time.time()
...
zm,zs=z_all.mean(axis=0),z_all.std(axis=0)
for g in TRAIN_G+[TEST_G]:
    z=get_z(wms[g],wm0,pca);zn=(z-zm)/(zs+1e-8)
    print(f"  g={g:4.1f}: z_raw=[{z[0]:+.4f},{z[1]:+.4f}] zn=[{zn[0]:+.3f},{zn[1]:+.3f}]")
print(f"  EVR: {pca.explained_variance_ratio_}")
print("\n[4] Train SepCondPolicy (50 ep)")
...
for ep in range(1,51):
    ...
    for _ in range(60):
        ...
        z=get_z(wms[g],wm0,pca);zn=(z-zm)/(zs+1e-8)
        zt=torch.tensor(zn,dtype=torch.float32).unsqueeze(0)
...
for g,label in [(9.8,"trained"),(TEST_G,"UNSEEN"),(19.6,"trained")]:
    r=eval_sep(policy,wm0,wms[g],pca,z_info,g=g,n=20)
