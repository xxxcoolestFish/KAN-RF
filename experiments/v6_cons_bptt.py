# CDPN v6: Consistency warmup + low-weight BPTT fine-tuning on Acrobot
...
print("="*70)
print("Consistency warmup + BPTT weight sweep on Acrobot")
...
for ep in range(1,11):
    ...
    for _ in range(100):
        ...
        loss.backward();torch.nn.utils.clip_grad_norm_(policy.parameters(),10.)
        opt.step();tl+=loss.item()
g15_pre=evaluate(policy,g=14.7);g9_pre=evaluate(policy,g=9.8)
print(f"  Consistency: loss={tl/100:.6f}  g=14.7={g15_pre}/20  g=9.8={g9_pre}/20")

S_G=torch.tensor([[-1.,0.,1.,0.,0.,0.]]).to(DEVICE)
print("\n[2b] Sweep BPTT weights (20 epochs each)")
results={}
for lam in [1e-5, 3e-5, 1e-4, 3e-4, 1e-3]:
    ...
    for ep in range(1,21):
        ...
        for _ in range(100):
            ...
            with torch.no_grad():
                a_old=policy0(s);s_target=wm0(torch.cat([s,a_old],-1))
            a_new=pi(s);s_pred=wm1(torch.cat([s,a_new],-1))
            cl=mse(s_pred,s_target);bl=torch.tensor(0.,device=DEVICE)
            sc=s.clone()
            for t in range(3):
                a=pi(sc);sc=wm1(torch.cat([sc,a],-1))
                bl=bl+(0.9**t)*(sc-S_G.expand(128,-1)).pow(2).sum(dim=-1).mean()
            loss=cl+lam*bl;loss.backward()
            torch.nn.utils.clip_grad_norm_(pi.parameters(),10.)
            op.step();tl_c+=cl.item();tl_b+=bl.item();tl+=loss.item()
        cbs+=tl_c;bbs+=tl_b
    g15=evaluate(pi,g=14.7);g9=evaluate(pi,g=9.8)
    avg_c=cbs/2000;avg_b=bbs/2000
    results[lam]=(g15,g9,avg_c,avg_b)
    print(f"  lam={lam:.0e}: g=14.7={g15}/20  g=9.8={g9}/20  | C_loss={avg_c:.6f} B_loss={avg_b:.4f}")

best=max(results,key=lambda k:results[k][0])
print(f"\n  Best lam={best:.0e} -> g=14.7={results[best][0]}/20 g=9.8={results[best][1]}/20")
print(f"  Consistency alone: g=14.7={g15_pre}/20 g=9.8={g9_pre}/20")
