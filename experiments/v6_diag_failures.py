"""Diagnose failure modes of State Consistency adaptation on Acrobot."""
...
def roll_one(policy,g=9.8,s0=None,max_steps=500):
    if s0 is None:
    ...
    traj={"states":[],"actions":[],"tip_h":[],"success":False}
    for st in range(max_steps):
        with torch.no_grad():a=policy(s).item()
        traj["states"].append(s[0].clone());traj["actions"].append(a)
        s=acrobot_step(s,torch.tensor([[a]]),g)
        h=tip_h(s).item();traj["tip_h"].append(h)
        if h>TARGET_H:traj["success"]=True;break
    traj["states"]=torch.stack(traj["states"])
    traj["actions"]=torch.tensor(traj["actions"])
    traj["tip_h"]=torch.tensor(traj["tip_h"]);traj["n_steps"]=st+1
    return traj

print("="*70)
print("Diagnosis: Consistency Adaptation Failure Modes on Acrobot")
...
print("\n[4] State Consistency Adaptation (10 epochs)")
...
for ep in range(1,11):
    ...
    for _ in range(100):
        ...
        loss.backward();torch.nn.utils.clip_grad_norm_(policy.parameters(),10.)
        opt.step();tl+=loss.item()
print(f"  Done. Final loss={tl/100:.6f}")
print("\n[5] Rolling 20 episodes on g=14.7 ...")
trajs=[roll_one(policy,g=14.7) for _ in range(20)]
succ=[t for t in trajs if t["success"]]
fail=[t for t in trajs if not t["success"]]
print(f"  Success: {len(succ)}/20  Failure: {len(fail)}/20")
fp=[t["tip_h"].max().item() for t in fail]
sp=[t["tip_h"].max().item() for t in succ]
print(f"\nPeak height: succ [{min(sp):.2f},{max(sp):.2f}] fail [{min(fp):.2f},{max(fp):.2f}]")
print(f"Failures below 0.5: {sum(1 for p in fp if p<0.5)} " +
      f"borderline 0.5-1.0: {sum(1 for p in fp if 0.5<=p<TARGET_H)}")
for i,t in enumerate(fail):
    a=t["actions"];s=t["states"];n=len(s)
    flips=((a[:-1]*a[1:])<0).sum().item()
    print(f"  Fail{i}: steps={n} mean|a|={a.abs().mean():.4f} " +
          f"flips={flips} peak_h={t['tip_h'].max():.3f}")
for i,t in enumerate(succ[:5]):
    a=t["actions"];flips=((a[:-1]*a[1:])<0).sum().item()
    print(f"  Succ{i}: steps={t['n_steps']} mean|a|={a.abs().mean():.4f} " +
          f"flips={flips} peak_h={t['tip_h'].max():.3f}")
for i,t in enumerate(fail+succ[:5]):
    s=t["states"];a=t["actions"];v1=s[:,4]
    cc=np.corrcoef(a.abs().numpy(),v1.abs().numpy())[0,1]
    label="S" if t["success"] else "F"
    print(f"  {label}{i}: |a|-|v1| corr={cc:.3f}")
with torch.no_grad():
    for i,t in enumerate(fail+succ[:5]):
        s=t["states"];n=len(s);da=(policy(s)-policy0(s)).abs()
        hd=da[:min(50,n)].mean().item();td=da[-min(50,n):].mean().item()
        label="S" if t["success"] else "F"
        print(f"  {label}{i}: pi-pi0 head50={hd:.4f} tail50={td:.4f}")
mf=sum(t["actions"].abs().mean().item() for t in fail)/len(fail) if fail else 0
ms=sum(t["actions"].abs().mean().item() for t in succ)/len(succ) if succ else 0
print(f"\nMean |a|: fail={mf:.4f} succ={ms:.4f} ratio={mf/ms if ms>0 else 0:.3f}")
with torch.no_grad():
    df=sum((policy(t["states"])-policy0(t["states"])).abs().mean().item() for t in fail)/len(fail) if fail else 0
    ds=sum((policy(t["states"])-policy0(t["states"])).abs().mean().item() for t in succ)/len(succ) if succ else 0
print(f"Mean |pi-pi0|: fail={df:.4f} succ={ds:.4f} ratio={df/ds if ds>0 else 0:.3f}")
