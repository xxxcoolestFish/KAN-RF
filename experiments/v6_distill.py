# CDPN v6: Grid search distillation on Acrobot
...
def grid_labels(wm,states,n_grid=41):
    """Grid search: a* = argmin ||WM(s,a) - s_goal||^2"""
    ...
    with torch.no_grad():
        ...
        # Combined: state distance + tip_height change
        sd=(s_next-S_G.view(1,1,6)).pow(2).sum(dim=-1)
        h_next=tip_h(s_next.view(-1,6)).view(batch,n_grid)
        # Use tip_height change (direction-sensitive) with state distance regularization
        score=-sd+0.5*h_next
        _,idx=score.max(dim=-1)
    return a_grid[idx].squeeze(-1)

print("="*70)
print("Grid Search Distillation on Acrobot")
...
policy_con=ProtoPolicy();policy_con.load_state_dict(policy0.state_dict())
opt=torch.optim.Adam(policy_con.parameters(),lr=1e-3)
for ep in range(1,11):
    tl=0.0
    for _ in range(100):
        s=random_states(128);opt.zero_grad()
        with torch.no_grad():
            a_old=policy0(s);s_target=wm0(torch.cat([s,a_old],-1))
        a_new=policy_con(s);s_pred=wm1(torch.cat([s,a_new],-1))
        loss=mse(s_pred,s_target)
        loss.backward();torch.nn.utils.clip_grad_norm_(policy_con.parameters(),10.)
        opt.step();tl+=loss.item()
g15_c=evaluate(policy_con,g=14.7);g9_c=evaluate(policy_con,g=9.8)
print(f"  Consistency: g=14.7={g15_c}/20  g=9.8={g9_c}/20")

# Evaluate: consistency + grid search distillation
results=[]
for label in ["GridDistill"]:
    pi=ProtoPolicy();pi.load_state_dict(policy_con.state_dict())
    op=torch.optim.Adam(pi.parameters(),lr=1e-4)
    print(f"\n[2b] {label} (20 epochs)")
    for ep in range(1,21):
        tl=0.0
        for _ in range(100):
            s=random_states(128)
            a_star=grid_labels(wm1,s)
            op.zero_grad()
            loss=mse(pi(s),a_star.unsqueeze(-1))
            loss.backward();torch.nn.utils.clip_grad_norm_(pi.parameters(),10.)
            op.step();tl+=loss.item()
        if ep%10==0:
            g15=evaluate(pi,g=14.7);g9=evaluate(pi,g=9.8)
            print(f"    ep{ep:2d} g=14.7={g15}/20  g=9.8={g9}/20  loss={tl/100:.6f}")
    g15=evaluate(pi,g=14.7);g9=evaluate(pi,g=9.8)
    results.append((label,g15,g9))
    print(f"  Result: g=14.7={g15}/20  g=9.8={g9}/20")
...
print(f"  BPTT: g=14.7={g15_f}/20  g=9.8={g9_f}/20")
...
for label,g15,g9 in results:print(f"  {label:30s}  {g15:8d}  {g9:8d}")
