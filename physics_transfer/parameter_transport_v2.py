"""Stable parameter-space transport used by the stage-27 validation."""
...
class ParameterTransport(nn.Module):
    """Full-vector transport with robust per-parameter normalization."""
    ...
    def __init__(self, cognitive: nn.Module, layer_shapes: Iterable[tuple[int, int]],
        ...
        # A hard floor prevents tiny ProtoKAN spline coefficients from making
        # online parameter updates saturate the transport encoder.
        ...
        self.encoder = nn.Sequential(nn.Linear(self.theta_dim, hidden_dim), nn.Tanh())
    ...
    def forward(self, cognitive: nn.Module):
        ...
        hidden = self.encoder(theta / self.reference_scale)
...
class TransportedMLPPolicy(nn.Module):
    def __init__(self, cognitive: nn.Module, transport: ParameterTransport,
        ...
        self.in_dim, self.hidden_dim = state_dim + goal_dim, hidden_dim
    ...
    def forward(self, state, goal, use_transport: bool = True):
        ...
        if use_transport:
            ...
            updates = [torch.zeros_like(self.base_weight_1),
                       torch.zeros_like(self.base_weight_2)]
