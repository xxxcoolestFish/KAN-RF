...
action space, together with a structured KAN realization that exposes the
required local action-effect operator.  Third, we combine the pullback with
...
task-relevant directions.  The validated base implementation uses $W=I$.
Learning or deriving a nontrivial task-relevant metric is treated as a pending
extension and is not used in the reported claims.
...
recover a frozen source policy, (ii) how rapidly a self-supervised online KAN
approaches that oracle, and (iii) whether recurring dynamics can be recalled
without catastrophic performance loss.  The policy is trained under one source
...
reports five-seed means.  The oracle pullback uses the true target dynamics;
the online result uses only transition updates.

\begin{table}[t]
\centering
\caption{CartPole success rate under unseen dynamics. ``Online'' uses 2,048
target transitions and keeps the actor frozen.}
\label{tab:cartpole}
\begin{tabular}{lrrr}
\toprule
Shift & Frozen & Oracle & Online \\
\midrule
Heavy cart + weak actuator & 0.00 & 100.00 & 100.00 \\
High gravity + weak actuator & 0.00 & 99.80 & 99.86 \\
Hard multi-parameter shift & 0.00 & 96.78 & 92.13 \\
\bottomrule
\end{tabular}
\end{table}

For the hard multi-parameter shift, success rises from 59.41\% with 64 target
transitions to 78.75\%, 80.61\%, 81.52\%, 88.75\%, and 92.13\% at budgets
$128,256,512,1024,$ and $2048$, respectively.  All five seeds exceed 90\% at
the final budget.  This curve supports the intended causal chain: as the
transition-only cognitive posterior improves, the frozen policy is re-grounded
...
\begin{tabular}{lrr}
\toprule
Metric & Spectral baseline & PCET \\
\midrule
Mean recovery curve & 75.89 & \textbf{82.76} \\
Final success mean & 95.55 & \textbf{95.70} \\
Worst final success & 71.48 & \textbf{72.66} \\
Coupled-shift recovery & 44.69 & \textbf{60.31} \\
Seeds reaching 95\% on coupled shift & 1/5 & \textbf{3/5} \\
\bottomrule
\end{tabular}
\end{table}
...
The current evidence establishes feasibility and cross-system recovery, but
the final empirical claim requires a unified-budget comparison with (i) learned
inverse dynamics/GAT-style action transfer, (ii) a context-conditioned actor,
(iii) online policy fine-tuning, (iv) model-predictive control, and (v) an
equal-capacity MLP forward model.  The decisive ablation compares $W=I$ with a
task-relevant effect metric and removes posterior risk, stable transport, and
non-overwriting memory individually.  These comparisons are deliberately not
filled with proxy numbers in this draft.
...
memory, and the two-link memory can create duplicate contexts.  The base
pullback matches the full chosen effect ($W=I$); whether a learned
task-relevant effect metric improves transfer over full-effect matching remains
an open experiment.  Finally, local control affinity and sufficient excitation
...
under the current dynamics.  Initial results show rapid source-only recovery in
systems with different action dimensions.  The remaining decisive question is
whether task-relevant posterior transport consistently outperforms strong
inverse-dynamics and context-conditioning baselines under identical online
budgets.
