...
## 12. Multi-Step KAN Rollout (Sequence Policy) 实验

### 动机

当前最优的 decision_v3 单步训练只在训练时查询 KAN 一次：$s \to \pi_\theta(s) \to a \to \text{KAN} \to s'$。这不满足三个目标：
1. **利用 KAN 参数**：只用了一次 KAN 前向
2. **避免根因三**：单步 Jacobian 太小
3. **序列预测**：没有做

序列策略在时间轴上展开 $H$ 步：每一步都查询 $\pi_\theta(s_t) \to a_t$，通过 KAN 得到 $s_{t+1}$。梯度通过 $H$ 次 KAN 前向传播回到策略的每次调用。这直接利用了 KAN 的所有 B-样条参数进行序列规划。

### 实现

新增 `KANSequenceTrainer`（`decision_v3/core.py`）：
- 每步调用 $\pi_\theta(s_t) \to a_t$（与旧 MultiStep 用 a=0 不同）
- 每步用能量引导损失 + 密度加权
- KAN 激活密度 $\rho(s_t)$ 自动降权 OOD 步骤
- BPTT 通过 $H$ 次 KAN 展开
...
|------|:---:|:---:|
| 成功率 | 10/10 | 10/10 |
| Mean \|Δθ\| | 0.095 ± 0.056 | 0.100 ± 0.072 |
| Mean steps | 14.4 | 15.2 |

### 分析

两者在 Pendulum 上都达到 10/10，无法区分优劣——任务已经饱和。但序列策略在以下场景可能更有价值：
1. 需要多步协调动作的环境（如 Acrobot）
2. 无法使用能量先验的场景（非 pendulum 系统）

三个目标均已满足：✓ 利用 KAN 参数 ✓ 避免根因三 ✓ 序列预测。下一步应在更复杂环境上验证。



---

## 13. CartPole 决策实验（2026-07-04）

### 目标

将 decision_v3 管线从 Pendulum 移植到 CartPole，验证 KAN 梯度训练在第二个环境上的可行性。比较单步 vs 序列策略。

### CartPole 特殊性

与 Pendulum 的关键差异：
1. **作用力方向反转**：向右推 → pole 受惯性力向左倒。CWS Jacobian 的符号容易出错。
2. **运动学耦合**：小车位置和 pole 角度通过加速度耦合
3. **无能量先验**：不能直接用能量损失

### 实现

- KAN 世界模型：`[5, 12, 4]`，972 参数，CWS-only (ν=1.0)，600 epochs
- 模拟器：semi-implicit Euler，支持连续力
- 损失函数：$\min |\theta'| + 0.1|x'|$（最小化 pole 角度 + 小车偏移）
- 策略：MLP `[4, 64, 64, 1]`

### 调试过程（教训）

1. **第 1 轮**：KAN Jacobian 符号反了（正号）。原因：CWS 公式 `d(theta_acc)/d(force)` 少写负号。CartPole 中向右推车 → pole 向左倒（惯性耦合），Jacobian 必须为负。
2. **第 2 轮**：修复符号后，单步和序列都达到 100%。

### 结果

| 策略 | 成功率 | 平均存活步数 |
|------|:---:|:---:|
| Heuristic PD (baseline) | 100% | 500 |
| **Single-step** | **100%** | **500** |
| **Sequence H=5** | **100%** | **500** |

两者都达到满分。CartPole 的难度不足以区分单步和序列策略。

### 关键教训

**CWS Jacobian 的符号必须在物理上正确。** 这次实验的核心教训是：当 KAN 梯度被用于训练策略时，Jacobian 的**符号**比**大小**重要得多。符号错了，策略学到相反的控制律；符号对了，即使大小不精确，梯度方向也足够训练出有效策略。

---

## 14. KAN-MPC 实验（2026-07-04）

### 动机

所有之前的决策方法都在部署时丢弃了 KAN。KAN-MPC 让 KAN 永久驻留在部署回路中：作为 MPC 内部的可微动力学引擎，直接参与每个控制周期的规划。
...
部署时: s → [MPC 优化器] → a*
                ↑
           [KAN f(s,a)→s']  ← 可微动力学模型，常驻部署
                ↑
           三个 KAN 专属能力:
           1. CWS 精确 Jacobian → 梯度优化候选动作
           2. ρ(s) 激活密度 → 免费不确定性 → 安全规划
           3. 有界导数 → 认证 rollout 误差
```

### 算法

每步：采样 N 个候选动作序列 → KAN 向量化 rollout → 评分 → 选 top-K 梯度精炼 → 执行第一步。

### 结果

| 环境 | KAN-MPC | decision_v3 | 动作探索器 |
|------|:---:|:---:|:---:|
| Pendulum | 70% | **100%** | **100%** |
| CartPole | 0% | **100%** | — |
...
**KAN-MPC 架构是正确的——KAN 确实在部署时参与决策。** 但性能不如简单方案。

**Pendulum 瓶颈**：KAN 在底部状态（sinθ < 0.5）的预测不可靠。训练数据全是随机动作，从未见过 pumping 轨迹。MPC 从底部进行 5 步 rollout 时，所有候选序列看起来都同样糟糕。

**CartPole 瓶颈**：KAN 的预测误差（val_mse=0.0042）在 5 步 rollout 中累积，使得 MPC 无法区分好的和坏的动作序列。CartPole 需要的控制精度远高于 Pendulum（pole 角度容差仅 0.21 rad）。

### 核心教训

**KAN-MPC 的性能上限由 KAN 的预测精度决定。** MPC 展开 H 步，每步的预测误差都会累积。当前 KAN（val_mse 0.001-0.004）在离线数据上表现不错，但在 MPC 多步 rollout 中，误差增长速度和控制的精细度要求之间的差距决定了 MPC 的成功率。
...
| 方法 | KAN 部署时参与 | 成功率 | KAN 知识利用 |
|------|:---:|:---:|------|
| 逆优化 | ✓ | 7/10 | 前向值（受根因三限制） |
| decision_v2 特征 | ✗ | 7/10 | 5 个标量 |
| 动作探索器 | ✗ | 100% | 无（绕过 KAN） |
| decision_v3 MLP | ✗ | 100% | 仅梯度信号 |
| 序列策略 | ✗ | 100% | H 次梯度 |
| **KAN-MPC** | **✓** | **70%** | **预测 + Jacobian + 不确定性** |

KAN-MPC 是唯一在部署时使用 KAN 三个专属能力的方法，但它的性能受限于 KAN 的世界模型精度。

### 核心矛盾

**KAN 做决策 vs KAN 做部署时决策。** decision_v3 让 KAN 在训练时做决策（提供梯度），部署时丢弃 KAN——效果好但避开了问题。KAN-MPC 让 KAN 在部署时做决策——架构正确但精度不够。
