...
这是当前最重要的实验，因为它决定能否合理声称 KAN 和控制等价接口的独立贡献。

### P0：修复两类稳定性边界

优先研究：

- `payload_125`：源策略已经很好时，如何自动判断无需适应；
- `friction_070`：为什么早期改善、后期退化。
...
- 更新前后 \(T_t\) 与 \(W_t\) 的谱；
- 每个预算点的动作修正幅值和方向；
- 是否存在某些 Hopper 相位主导后期更新。

不要先加入新的手工门控。先找到漂移发生在哪个算子、哪个状态区域和哪个时间段。
...
建议选择与 Hopper 接触结构不同、但仍是连续控制的环境，例如 HalfCheetah 或 Walker2d。目标不是立即追求最强结果，而是验证控制仿射接口是否可迁移。

### P1：论文同步

完成 P0 消融后立即重写论文方法和实验部分，不要等所有实验结束后再改。每个主张都要绑定：
...
但项目尚未越过“论文贡献已经被严格隔离”的阶段。当前最关键的问题不是再增加复杂模块，而是回答三个问题：

1. ProtoKAN 目标算子是否比参数匹配的普通在线模型更好？
2. 如何在无需适应和长期递归更新时避免负迁移与漂移？
3. 当决策网络也允许持续学习时，认知运输能否仍显著提高早期恢复速度？

如果这三个问题得到扎实答案，项目就具备形成有竞争力论文的基础。
