"""Compare low-rank physics coefficient capacities with counterfactual supervision."""
...
def train(rank: int, steps: int, seed: int):
    torch.manual_seed(seed)
    model = LowRankSplitCognitivePredictor(
        physics_rank=rank, residual_scale=0.1, state_dim=6,
    ...
    for index in range(steps):
        ...
        loss = (
            standard + absolute + difference + 0.01 * consistency
            + 0.05 * orthogonality + 0.1 * energy
        )
...
def main():
    ...
    results = []
    for rank in (2, 4, 6):
        model = train(rank, args.steps, args.seed)
        metrics = evaluate(model)
        metrics["physics_rank"] = rank
