"""Train low-rank coefficients with explicit trajectory invariance."""
...
def train(coeff_weight: float, steps: int, seed: int):
    ...
    for index in range(steps):
        ...
        coefficient_consistency = F.mse_loss(
            same_out_a["physics_coefficients"],
            same_out_b["physics_coefficients"],
        )
        horizon = rollout_loss(model, batch, horizon=4)
        loss = (
            standard + absolute + 50.0 * gap_loss
            + 0.01 * token_consistency + coeff_weight * coefficient_consistency
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        different = sample_multifactor_batch(128, 8, (HELDOUT[1],))
        out_same_a = model(same_a["history"], same_a["state"], same_a["action"])
        out_same_b = model(same_b["history"], same_b["state"], same_b["action"])
        out_different = model(
            different["history"], different["state"], different["action"]
        )
        return {
            "one_step_loss": rollout_loss(model, heldout, 1).item(),
            "four_step_loss": rollout_loss(model, heldout, 4).item(),
            "same_factor_coefficient_mse": F.mse_loss(
                out_same_a["physics_coefficients"],
                out_same_b["physics_coefficients"],
            ).item(),
            "different_factor_coefficient_distance": (
                out_same_a["physics_coefficients"].mean(0)
                - out_different["physics_coefficients"].mean(0)
            ).norm().item(),
        }
...
def main():
    ...
    for weight in (0.0, 0.1, 0.5, 1.0):
        ...
        metrics["coefficient_consistency_weight"] = weight
