"""Ablation: expose cognitive delta and query to the decision head.

The complete ProtoKAN prediction remains mandatory.  The head receives
``[predicted_next, predicted_next-state, query, goal]`` instead of only
``[predicted_next, goal]``; no raw-state shortcut is added.
"""
...
class CognitiveDeltaGaussianActor(GaussianActorBase):
    def __init__(self, cognitive: nn.Module, hidden_dim: int = 64):
        super().__init__(hidden_dim)
        self.cognitive = cognitive
        self.query_net = _mlp(STATE_DIM * 2, hidden_dim, ACTION_DIM)
        self.decision_head = _mlp(STATE_DIM * 3 + ACTION_DIM, hidden_dim, ACTION_DIM)
    ...
    def mean_action(self, state: torch.Tensor, goal: torch.Tensor):
        ...
        features = torch.cat([predicted_next, delta, query, goal], dim=-1)
...
base.CognitiveEmbeddedGaussianActor = CognitiveDeltaGaussianActor
