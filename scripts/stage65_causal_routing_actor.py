"""First direct policy whose action must pass through ProtoKAN causal routes."""
...
class ProtoKANCausalRoutingActor(GaussianActorBase):
    """Proposal -> cognitive trajectory -> causal route -> direct action."""
    ...
    def __init__(self, cognitive, router, horizon=8, hidden_dim=64,
        ...
        # No raw state and no proposal action enter this head.  Its inputs are
        # exclusively cognitive predictions/routes plus the task goal.
        feature_dim = 3 + 6 + 6 + 6
    ...
    def causal_features(self, state, goal):
        ...
        routes, predicted_states, scores, weights = temporal_causal_route(
            self.cognitive, state, proposal, self.temperature, self.router,
        )
        first_route = routes[:, 0]
        mean_route = routes.mean(dim=1)
        route_strength = routes.abs().mean(dim=1)
        final_state = predicted_states[:, -1]
        reachable_state = (
            weights.unsqueeze(-1) * predicted_states
        ).sum(dim=1)
        features = torch.cat([
            first_route, mean_route, route_strength,
            final_state, reachable_state, goal,
        ], dim=-1)
        return features
...
def train(args):
    ...
    actor = ProtoKANCausalRoutingActor(
...
def main():
    ...
    parser.add_argument("--policy-iterations", type=int, default=30)
    ...
    output = {
        "architecture": "ProtoKAN_NonlinearCausalRouting_DirectActor",
