...
def run_seed(args, seed):
    ...
    with torch.no_grad():
        ...
        # finite_difference_input_effect is (B, input_dim, output_dim), while
        # the analytical Jacobian is (B, output_dim, input_dim).
        finite_jacobian = finite_difference_input_effect(cognitive, states, actions, args.epsilon).transpose(1, 2)
        exact_jacobian = finite_difference_input_effect(exact_transition, states, actions, args.epsilon).transpose(1, 2)
    result = {
        "seed": seed,
        "cognitive_fit": fit,
        "one_step": {
