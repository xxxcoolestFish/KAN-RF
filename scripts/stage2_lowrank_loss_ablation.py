"""Ablate counterfactual difference supervision for low-rank transport."""
...
HELDOUT = ((9.80, 0.04, 1.10, 0.90), (13.475, 0.06, 0.90, 1.10))
...
def train(difference_weight: float, steps: int, seed: int):
    ...
    for index in range(steps):
        ...
        loss = (
            standard + absolute + difference_weight * difference
            + 0.01 * consistency + 0.05 * orthogonality + 0.1 * energy
        )
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        return {
            ...
            "basis_orthogonality": out_a["basis_gram_error"].square().mean().item(),
        }
...
def main():
    ...
    results = []
    for weight in (1.0, 5.0, 20.0):
        metrics = evaluate(train(weight, args.steps, args.seed))
        metrics["difference_weight"] = weight
