"""Standard PPO upper-bound baseline without any cognition module.

The environment is the current continuous-action Acrobot variant, but the
decision network is an ordinary goal-conditioned Gaussian MLP actor plus a
value MLP. Episodes start near the hanging-down state, terminate on the same
tip-height success condition used by our experiments, and have a 500-step
evaluation budget.
"""
...
def reward_fn(state, next_state, action):
    ...
    success = height >= 1.0
    # Smooth height return plus a terminal success bonus. This is a standard
    # task reward; no energy teacher or cognitive-model target is used.
...
def main():
    ...
    result = {
        "architecture": "StandardPPO_DirectMLP_NoCognition",
        "config": vars(args),
        "result": train(args),
    }
